...
- node_modules is a SYMLINK to ~/claude-planet/node_modules (worktree convenience). electron-builder / any packaging step must resolve the REAL module tree, not follow/copy a dangling symlink; verify the packaged app contains three r185 + addons and simplex-noise.
...
Either way verify the bundle includes: three r185 + its addons, `simplex-noise`, and the `public/` assets (`models/`, `textures/`). Note `node_modules` in the worktree is a **symlink** to `~/claude-planet/node_modules` — electron-builder must resolve the real tree, not the symlink.
