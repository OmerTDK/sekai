...
async function pollCommits(project, state, out) {
  ...
  for (const line of stdout.split('\n')) {
    ...
    out.push({ project, kind: 'commit', id: parsed.hash, ts: parsed.ts, title: truncateTitle(parsed.subject) });
  }
...
async function pollPrs(project, state, out) {
  ...
  if (!state.githubRepo) {
    warnGh('no GitHub remote on one or more active projects (or `git remote` unavailable) — PR-merge events skipped for it');
    return;
  }
  ...
  try {
    stdout = await execFileText(
      'gh',
      ['pr', 'list', '--repo', state.githubRepo, '--state', 'merged', '--limit', String(GH_PR_LIMIT), '--json', 'number,mergedAt,title'],
      GH_TIMEOUT_MS
    );
  } catch (e) {
